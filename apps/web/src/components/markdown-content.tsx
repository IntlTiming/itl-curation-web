...
// Single shared renderer for markdown-via-Tiptap content wherever it's displayed (review
// notes, comment bodies, submission details page, and anywhere else in the future) - raw HTML
// passthrough is deliberately never enabled (no rehype-raw), since this content is
// reviewer/commenter-submitted text and this keeps it from ever becoming an XSS vector.
//
...
export function MarkdownContent({ markdown }: { markdown: string | null }) {
  if (!markdown) return <span className="text-muted-foreground text-xs">No content</span>;
  return (
